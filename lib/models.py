...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, alterdotd):
        golist = alterdotd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_alterdotd(alterdotd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_alterdotd(self, alterdotd, rec):
        import decimal
        import alterdotlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from alterdotd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from alterdotd, with every run
        ...
        # get/create, then sync payment amounts, etc. from alterdotd - Alterdotd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(alterdotd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from alterdotd! %s" % e)
            govobj.vote_delete(alterdotd)
            return (govobj, None)
    ...
    def vote_delete(self, alterdotd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(alterdotd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, alterdotd, signal, outcome):
        import alterdotlib
        ...
        output = alterdotd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = alterdotlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(alterdotd, signal)

    def sync_network_vote(self, alterdotd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = alterdotd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = ADOTD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 alterdot addr, non-multisig
            if not alterdotlib.is_valid_alterdot_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Alterdot address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Alterdot  restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (alterdotlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = ADOTD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not alterdotlib.is_valid_alterdot_address(addr, config.network):
    ...
    def hash(self):
        import alterdotlib
        return alterdotlib.hashit(self.serialise())
