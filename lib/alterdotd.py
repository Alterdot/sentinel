...
class AlterdotDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some alterdotd methods
    ...
    @classmethod
    def from_alterdot_conf(self, alterdot_dot_conf):
        from alterdot_config import AlterdotConfig
        config_text = AlterdotConfig.slurp_config_file(alterdot_dot_conf)
        creds = AlterdotConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = alterdotlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = alterdotlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = alterdotlib.blocks_to_seconds(diff)
