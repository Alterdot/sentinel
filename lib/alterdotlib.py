...
def is_valid_alterdot_address(address, network='mainnet'):
    ...
    alterdot_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Alterdot address
        return False

    if (address_version != alterdot_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('alterdot.conf')
