...
def has_alterdot_conf():
    import config
    import io

    valid_alterdot_conf = False

    # ensure alterdot_conf exists & readable
    #
    # if not, print a message stating that Alterdot  must be installed and
    # configured, including JSONRPC access in alterdot.conf
    try:
        f = io.open(config.alterdot_conf)
        valid_alterdot_conf = True
    except IOError as e:
        print(e)

    return valid_alterdot_conf
...
def main():
    ...
    if not has_alterdot_conf():
        print("Alterdot must be installed and configured, including JSONRPC access in alterdot.conf")
        sys.exit(1)
